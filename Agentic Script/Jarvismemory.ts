...
const JARVIS_MD_PATH = path.join(process.cwd(), "JARVIS.md");
...
export async function updateJarvisMemoryMd(summary: string): Promise<void> {
  const date = new Date().toISOString().split("T")[0];
  const time = new Date().toLocaleTimeString();
  const entry = `\n### [${date} ${time}] Update\n${summary}\n`;
  
  if (await fs.pathExists(JARVIS_MD_PATH)) {
    await fs.appendFile(JARVIS_MD_PATH, entry);
  } else {
    await fs.writeFile(JARVIS_MD_PATH, `# J.A.R.V.I.S. Enhanced Memory Log\n${entry}`);
  }
}
