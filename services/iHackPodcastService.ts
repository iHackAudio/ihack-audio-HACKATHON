...
[And write the entire 4-5 minute detailed script here with bracketed emotion tags like [thoughtful], [enthusiastic], [sarcastic] etc., targeting 900-1100 words of solo narrative under the #### TRANSCRIPT header.]
...
export async function generateIHackPodcast(
  ...
  // Resolve modelId. If it corresponds to 'PRO' or 'FLASH', map them; otherwise use as is.
  let modelId = MODELS.TEXT_PRO;
  if (modelType === 'PRO') {
    modelId = MODELS.TEXT_PRO;
  } else if (modelType === 'FLASH') {
    modelId = MODELS.TEXT_FLASH;
  } else {
    modelId = modelType; // Use the specified Gemini model directly
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      const finalScript = response.text || "";
      logApiRequest(
        `Full Podcast Script Generation (${mode === 'MULTI' ? 'Duo' : 'Mono'} Mode)`,
        modelId,
        'TEXT',
        {
          charsIn: context.length + IHACK_SYSTEM_INSTRUCTION.length + formatGuideline.length,
          charsOut: finalScript.length,
          nodeId,
          status: 'SUCCESS'
        }
      );
